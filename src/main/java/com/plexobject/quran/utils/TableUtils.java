...
public class TableUtils {
	/**
	 * Returns the preferred height for the given row. It loops across all
	 * visible columns and returns the maximal pref height of the rendering
	 * component. Falls back to the table's base rowheight, i f there are no
	 * columns or the renderers max is zeor.
	 * <p>
	 * 
	 * @param table
	 *            the table which provides the renderers, must not be null
	 * @param row
	...
	public static int getPreferredRowHeight(JTable table, int row) {
		...
		for (int column = 0; column < table.getColumnCount(); column++) {
			...
			System.out.println("Setting height "
					+ comp.getPreferredSize().height + " for " + row);
		}
